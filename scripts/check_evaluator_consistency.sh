...
if python3 -c "import vllm" 2>/dev/null; then
  ...
  $VLLM_CMD "$MODEL" \
    ...
    > "$PROJECT_ROOT/logs/vllm_evaluator_consistency.log" 2>&1 &
  ...
  for i in {1..720}; do
    if ! kill -0 $VLLM_PID 2>/dev/null; then
      ...
      echo "--- Tail of $PROJECT_ROOT/logs/vllm_evaluator_consistency.log ---"
      tail -n 35 "$PROJECT_ROOT/logs/vllm_evaluator_consistency.log" 2>/dev/null || echo "(No log file found)"
      echo "====================================================================="
      break
    fi
    ...
    if [ $((i % 2)) -eq 0 ]; then
      ...
      LAST_LOG=$(tail -n 2 "$PROJECT_ROOT/logs/vllm_evaluator_consistency.log" 2>/dev/null | tr '\n' ' ')
      if [ -n "$LAST_LOG" ]; then
  ...
  if [ $READY -eq 1 ]; then
    ...
    echo "--- Full contents / tail of $PROJECT_ROOT/logs/vllm_evaluator_consistency.log ---"
    tail -n 50 "$PROJECT_ROOT/logs/vllm_evaluator_consistency.log" 2>/dev/null || echo "(No log file found)"
    echo "====================================================================================="
  fi
